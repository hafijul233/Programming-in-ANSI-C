...
int main()
{
  ...
  for(i=0;i<100;i++){
    ...
    if(max_total<sum)max_total=sum;
    highest[i]=sum;
    ...
    //Max Subject Marks
     ...
     for(i=0;i<100;i++){
      if(max_sub[0]<marks[i][1])max_sub[0]=marks[i][1];
      if(max_sub[1]<marks[i][2])max_sub[1]=marks[i][2];
      if(max_sub[2]<marks[i][3])max_sub[2]=marks[i][3];
    }
    ...
    for(i=0;i<3;i++){
      ...
      printf("\tHighest on Subject%d:\nMarks: %d\nRolls: ",i+1,max_sub[i]);
        ...
        for(j=0;j<100;j++){
        if(marks[j][i] == max_sub[i])printf("%d\t",marks[j][0]);
      }
      printf("%d\t",max_sub[i]);
    }
